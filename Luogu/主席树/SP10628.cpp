...
int n,m,a[400010];
//LCA
int LCA_ST[400010][23],LCA_dep[400010];
inline void LCA_init(int k,int dep,int fath){
    LCA_ST[k][0] = fath;
    LCA_dep[k] = dep;
    for(int pppp=0;pppp<v[k].size();pppp++){
        int i = v[k][pppp];
        if(i==fath) continue;
        LCA_init(i,dep+1,k);
    }
}
inline void LST_ST_init(){
    for(int j = 1;j<23;j++){
        for(int i = 1;i<=n;i++){
...
inline int LCA_jump(int k,int dep){
    int tmp = 0;
    while(dep){
        if(dep&1){
            k = LCA_ST[k][tmp];
...
inline void change(int k,int p,int q){
...
inline int kth(int xroot,int yroot,int lcaroot,int fathroot,int c,int l,int r){
    if(l==r) return l;
...
signed main(){
	...
	int lst = 0;
    for(int i = 1;i<=m;i++){
        read(q[i].u);read(q[i].v);read(q[i].k);
