...
signed main(){
    ...
    while(T--){
		// if(T==9998){
		// 	read(n);write(n);write('\n');
		// 	for(int i = 1;i<=n;i++) read(a[i].x),write(a[i].x),write('\t');write('\n');
		// 	for(int i = 1;i<=n;i++) read(a[i].y),write(a[i].y),write('\t');write('\n');
		// 	read(m);write(m);write('\n');
		// 	for(int i = 1;i<=m;i++) read(b[i].x),write(b[i].x),write('\t');write('\n');
		// 	for(int i = 1;i<=m;i++) read(b[i].y),write(b[i].y),write('\t');write('\n');
		// 	return 0;
		// }
        read(n);
...
//         for(int i = 1;i<=n;i++) cout << tmp[i] << " ";cout << endl;
        // for(int i = 1;i<=n;i++) cout << lower_bound(b,b+1+m,(node){0,to[0][i],tmp[i]},cmp_y)-b << " ";cout << endl;
//		for(int i = 1;i<=n;i++) cout << to[0][i] << " ";cout << endl;
...
//		 for(int i = 1;i<=n;i++) cout << to[0][i] << " ";cout << endl;
		...
		// cout << "-----" << endl;for(int i = 1;i<=m;i++) cout << tmp[i] << " ";cout << endl << "-----" << endl;
		// cout << "-----" << endl;for(int i = 1;i<=m;i++) cout << to[1][i] << " ";cout << endl << "-----" << endl;
		// cout << "-----" << endl;for(int i = 1;i<=m;i++) cout << lower_bound(a,a+1+n,(node){0,to[1][i],tmp[i]},cmp_y)-a << " ";cout << endl << "-----" << endl;
        for(int i = 1;i<=m;i++) to[1][i] = to[1][i]?a[lower_bound(a,a+1+n,(node){0,to[1][i],tmp[i]},cmp_y)-a].id:0;
		// cout << "-----" << endl;for(int i = 1;i<=m;i++) cout << to[1][i] << " ";cout << endl << "-----" << endl;
//	    cout << b[72].x << " " << b[72].y << endl;
// 		cout << endl;
