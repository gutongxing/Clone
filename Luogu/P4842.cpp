...
inline void pushup(int id){
	...
	if(id) tree[id].sz = tree[tree[id].son[0]].sz+1+tree[tree[id].son[1]].sz;
}
...
inline void makeplus(int id,int k){
	if(!id) return;
	tree[id].sum += tree[id].sz*k;
}
inline void pushdown(int id){
	...
	if(tree[id].plus){
		makeplus(tree[id].son[0],tree[id].plus);
		makeplus(tree[id].son[1],tree[id].plus);
		tree[id].plus = 0;
	} 
}
...
signed main(){
	...
	for(int i = 1;i<=m;i++){
		...
		case 4:{
			...
			Access(y);
			splay(y);
			int p = tree[y].sum,q = tree[y].sz;
			int gcd = __gcd(p,q);
			write(p/gcd);putchar('/');write(q/gcd);putchar('\n');
			break;
		}
		case 2:{
